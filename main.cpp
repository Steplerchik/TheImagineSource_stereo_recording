...
GstFlowReturn new_frame_cb(GstAppSink *appsink, gpointer data)
{
    ...
    if( strcmp( gst_structure_get_string (str, "format"),"GRAY8") == 0)  
    {
        ...
        if (info.data != NULL) 
        {
            ...
            memcpy( pCustomData->frame.data, info.data, width*height*1);
...
int main(int argc, char **argv)
{
    ...
    TcamCamera cam1("1810488");    
    TcamCamera cam2("41710500");    
    
    // Set video format, resolution and frame rate
    cam1.set_capture_format("GRAY8", FrameSize{1280,960}, FrameRate{5,1});
    cam2.set_capture_format("GRAY8", FrameSize{1280,960}, FrameRate{5,1});
